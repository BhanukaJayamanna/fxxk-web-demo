                                    ...
                                    <a target="{{$item->is_newtab == 1?'_blank':''}}" href="{{route('frontend.category-product',['slug' => @$item->category->slug, 'item' =>'category'])}}" class="menu_product">
                                        <div class="single_mega_menu_product">
                                          <div class="media align-items-center">
                                            <div class="media_img">
                                              <img data-src="{{asset(asset_path(@$item->category->categoryImage->image?@$item->category->categoryImage->image:'frontend/default/img/default_category.png'))}}" src="{{showImage(themeDefaultImg())}}" class="lazyload align-self-center"alt="{{@$item->category->name}}"/>
                                            </div>
                                ...
                                <div class="product_logo">
                                  @foreach(@$menu->menu->bottomPanelData as $key => $item)
                                    ...
                                    <a target="{{$item->is_newtab == 1?'_blank':''}}" href="{{route('frontend.category-product',['slug' => @$item->brand->slug, 'item' =>'brand'])}}" class="single_product_logo">
                                      <img data-src="{{ asset(asset_path(@$item->brand->logo ? @$item->brand->logo:'frontend/default/img/brand_image.png')) }}" src="{{showImage(themeDefaultImg())}}" class="lazyload" alt="{{@$item->brand->name}}" />
                                    </a>
                                    @if($key >=7)
                                      @break
                                    @endif
                ...
                <div class="single_banner_slider">
                  <div class="row align-items-center">
                    ...
                    " {{$slider->is_newtab == 1?'target="_blank"':''}} class="slider_img_div">
                      <img data-src="{{asset(asset_path($slider->slider_image))}}" src="{{showImage(themeDefaultImg())}}" class="lazyload" alt="{{$slider->name}}">
                    </a>
                  </div>
                </div>
                @endforeach
                ...
                <a {{$item->is_newtab == 1?'target="_blank"':''}} href="{{route('frontend.category-product',['slug' => $item->category->slug, 'item' =>'category'])}}">
                  <div class="single_product_item_iner">
                    <div class="header_img_category_div">
                        <img
                        data-src="{{asset(asset_path(@$item->category->categoryImage->image?@$item->category->categoryImage->image:'frontend/default/img/default_category.png'))}}"
                        alt="{{$item->title}}"
              ...
              @foreach($headerProducts as $key => $item)
                <div class="single_banner_product product_price">
                  <a {{$item->is_newtab == 1?'target="_blank"':''}} href="{{singleProductURL(@$item->product->seller->slug, $item->product->slug)}}" class="product_img">
                    <img
                      data-src="{{asset(asset_path(@$item->product->product->thumbnail_image_source))}}"
                      alt="#"
